...
Route::group(['prefix' =>'admin', 'middleware' => ['auth', 'admin']], function(){
    ...
    Route::get('/products/jigijog','ProductController@jigijog_products')->name('products.jigijog');
	...
	Route::post('/sellers/id_modal', 'SellerController@id_modal')->name('sellers.id_modal');
	Route::post('/sellers/payment_modal', 'SellerController@payment_modal')->name('sellers.payment_modal');
	Route::post('/sellers/referral_modal', 'SellerController@payment_modal_form')->name('sellers.payment_modal_form');
	Route::post('/sellers/payment_method_form', 'SellerController@payment_method_modal')->name('sellers.payment_method_modal');
	...
	Route::get('/orders_general', 'OrderController@general_orders')->name('order.general.admin');
	Route::get('/orders/{id}/show', 'OrderController@show')->name('orders.show');
	Route::get('/orders_general/{id}/show', 'OrderController@general_show')->name('general.show');
    ...
    //admin_wallet
	Route::get('wallet_withdrawals/','WalletController@admin_index')->name('wallet.admin_index');
	Route::post('/payments', 'WalletController@payments')->name('wallets.payments');
	Route::get('admin_jigijog_fee_wallet/','WalletController@admin_wallet_index')->name('wallet.admin_wallet_index');
	
	//Referral
    // 	Route::resource('referral_program','ReferralProgramController');
    Route::get('referral_program/customer', 'ReferralProgramController@admin_customer_index')->name('referral_program.admin_customer_index');
	Route::get('referral_program/seller','ReferralProgramController@admin_seller_index')->name('referral_program.admin_seller_index');
	Route::get('referral_program/id_verification','ReferralProgramController@admin_id_verify')->name('referral_program.admin_id_verify');
	Route::get('/referrals/approve/{id}', 'ReferralProgramController@approve_referral')->name('referral_program.approve');
	Route::get('/referrals/reject/{id}', 'ReferralProgramController@reject_referral')->name('referral_program.reject');
	
	//All Users
	Route::get('/user_list', 'UserController@index')->name('user_list.user_list_index');
	Route::get('/user_list/{id}', 'UserController@verify')->name('user_list.verify');
	
	//for pickup
	Route::get('/pickup', 'PickupController@index')->name('pickup_list.index');
	Route::get('/pickup/{order_id}/{seller_id}', 'PickupController@view')->name('pickup_list.view');
	
	//for receipts upload
	Route::get('/receipts', 'ReceiptController@index')->name('receipt.index');
	Route::post('/receipts/upload', 'ReceiptController@image_upload')->name('receipt.upload');
	
	//for jigijog sales report
	Route::get('/jigijog_sales_report', 'JigijogSalesController@index')->name('jigijog.index');
	
	//for expenses
	Route::get('/expenses', 'ExpenseController@index')->name('expense.index');
	Route::get('/expenses/add', 'ExpenseController@create')->name('expense.create');
	Route::post('/expenses/store', 'ExpenseController@store')->name('expense.store');
