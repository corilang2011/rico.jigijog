...
Route::post('/ajax-prod-search', 'HomeController@ajax_prod_search')->name('prod_search.ajax');
...
Route::group(['middleware' => ['user', 'verified']], function(){
	...
	Route::post('/wallet/details', 'WalletController@wallet_details')->name('wallet.details');
	Route::post('/recharge', 'WalletController@recharge')->name('wallet.recharge');

	Route::resource('support-ticket','SupportTicketController');
	Route::post('support-ticket/reply','SupportTicketController@seller_store')->name('support-ticket.seller_store');
	
    Route::resource('referrals','ReferralProgramController');
});
...
Route::group(['middleware' => ['auth']], function(){
	...
	Route::get('invoice/seller/print/{order_id}', 'InvoiceController@seller_invoice_print')->name('seller.invoice.print');

	Route::resource('orders','OrderController');
	Route::post('/orders/accept', 'OrderController@add_pickup_date')->name('orders.add.pickdate');
	Route::get('/orders/destroy/{id}', 'OrderController@destroy')->name('orders.destroy');
	Route::post('/orders/details', 'OrderController@order_details')->name('orders.details');
	Route::post('/orders/update_delivery_status', 'OrderController@update_delivery_status')->name('orders.update_delivery_status');
	Route::post('/orders/update_payment_status', 'OrderController@update_payment_status')->name('orders.update_payment_status');

	Route::post('/orders/update_delivery', 'OrderController@update_delivery')->name('orders.update_delivery');
	Route::post('/orders/update_payment', 'OrderController@update_payment')->name('orders.update_payment'); 
	
	//start route send email update delivery notification by ricolangue 10/07/2019
	Route::post('/orders/delivery_notification', 'OrderController@delivery_notification')->name('delivery_notification');  
	//Route::post('/orders/delivery_notification',   'DeliveryNotification@delivery_notification')->name('test_email');   
	Route::post('/orders/delivery_paid_notification', 'OrderController@delivery_paid_notification')->name('delivery_paid_notification');  
	Route::post('/orders/new_delivery_notification', 'OrderController@new_delivery_notification')->name('new_delivery_notification');
	//end route send email update delivery notification by ricolangue 10/07/2019 
	
	//orders update for pickup
	Route::post('/orders/update_delivery', 'OrderController@update_delivery')->name('orders.update_delivery');
	Route::post('/orders/update_payment', 'OrderController@update_payment')->name('orders.update_payment');
	
	Route::post('/orders/decline', 'OrderController@decline_order')->name('orders.decline_order');
...
Route::get('referrals/customer','ReferralProgramController@customer_index');//Reset Password ResentRoute::get('password/email/resent', 'Auth\ForgotPasswordController@reset_email_sent')->name('password.email.resent');

Route::post('/ajax/province', 'AjaxController@index');
Route::post('/ajax/barangay', 'AjaxController@barangay');