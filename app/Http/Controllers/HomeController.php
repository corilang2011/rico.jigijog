...
class HomeController extends Controller
{
    ...
    public function ajax_prod_search(Request $request)
    {
        $keywords = array();
        $products = filter_products(Product::where('user_id', Auth::user()->id)->where('name', 'like', '%'.$request->search.'%'))->get();
        if(sizeof($products)>0){
            return view('frontend.partials.prod_search_content', compact('products', 'keywords'));
        }
        return '0';
    }
