...
@section('content')
    ...
    <div class="panel">
        <div class="panel-body">
            ...
            <div class="row">
                @php
                    $delivery_status = $order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->first()->delivery_status;
                    $payment_status = $order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->first()->payment_status;
                @endphp
                ...
                <div class="col-lg-3">
                    @if ($delivery_status == 'pending' || $delivery_status == 'on_review' || $delivery_status == 'on_delivery')
            ...
            <div class="invoice-bill row">
                ...
                <div class="col-sm-6 text-xs-center">
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            {{ single_price($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('price') + $order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('tax')) }}
                        </td>
                    </tr>
                    <tr>
                        <td class="text-main text-bold">
            ...
            <div class="row">
                <div class="col-lg-12 table-responsive">
                    <table class="table table-bordered invoice-summary">
                        ...
                        <tbody>
                            @foreach ($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id) as $key => $orderDetail)
                                @php
            ...
            <div class="clearfix">
                ...
                <tr>
                    ...
                    <td>
                        {{ single_price($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('price')) }}
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        {{ single_price($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('tax')) }}
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        {{ single_price($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('shipping_cost')) }}
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td class="text-bold h4">
                        {{ single_price($order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('price') + $order->orderDetails->sum('tax') + $order->orderDetails->where('order_id', $order->id)->where('seller_id', $seller_id)->sum('shipping_cost')) }}
                    </td>
                </tr>
                </tbody>
                </table>
