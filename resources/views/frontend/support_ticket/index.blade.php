...
<title>JigiJog | Support Ticket</title>
@section('content')
                                ...
                                <div class="col-md-6">
                                    <div class="float-md-right">
                                        <ul class="breadcrumb">
                                            ...
                                            <li><a href="{{ route('support-ticket.index') }}">{{__('Support Ticket')}}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    @if(count($tickets) > 0)
                                        @foreach ($tickets as $key => $ticket)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{route('support-ticket.show', encrypt($ticket->id))}}" class="btn btn-styled btn-link py-1 px-0 icon-anim text-underline--none">
                                                        {{__('View Details')}}
                                                ...
                                                @if($ticket->replied == 1)
                                                    <td>
                                                        <span class="badge badge-pill badge-info " >New Message</span>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
        <div class="modal-content position-relative">
            ...
            <div class="modal-body px-3 pt-3">
                <form class="" action="{{ route('support-ticket.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group mb-2">
                        <label class="text-danger"><i><strong>Note:</strong> Please construct and specify your problem properly before sending a ticket in order for us to address it quickly.</i></label>
                    </div>
